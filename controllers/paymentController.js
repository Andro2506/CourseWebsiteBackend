...
export const buySubscription = catchAsyncError(async (req, res, next) => {
    ...
    const plan_id = process.env.PLAN_ID || "plan_M7o1dXNsQlPf89";
...
export const paymentVerification = catchAsyncError(async (req, res, next) => {
    ...
    const isSignMatch = generated_signature === razorpay_signature;

    if (!isSignMatch) return res.redirect(`${process.env.FRONTEND_URL}/paymentfailed`);
...
export const cancelSubscription = catchAsyncError(async (req, res, next) => {
    ...
    res.status(200).json({
        success: true,
        message: refund ? "Subscription cancelled! You will receive full refund within 7 days." : "Subscription cancelled!, but refund will not be provided as you have cancelled your subscription after 7 days of your payment.",
    })
})