...
export const updateProfile = catchAsyncError(async (req, res, next) => {
    ...
    if (!name && !email) return next(new ErrorHandler("Please enter atleast one field!", 400));
...
export const forgetPassword = catchAsyncError(async (req, res, next) => {
    ...
    await sendEmail(user.email, "CorseWebsite Reset Password", message);
...
export const resetPassword = catchAsyncError(async (req, res, next) => {
    ...
    if (!user) return next(new ErrorHandler("Token has been expired!"));
...
export const addToPlaylist = catchAsyncError(async (req, res, next) => {
    ...
    if (itemExist) return next(new ErrorHandler("Item alredy exist!", 409));

    user.playlist.push({
        course: course._id,
        poster: course.posters.url,
    })
